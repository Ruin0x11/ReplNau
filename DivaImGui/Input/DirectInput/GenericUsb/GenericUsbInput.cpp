...
#include "../../KeyConfig/Config.h"
#include <stdio.h>

const std::string GU_CONFIG_FILE_NAME = "keyconfig_repl.ini";
...
namespace ReplNau::Input
{
	...
	bool GenericUsbInput::Initialize()
	{
		...
		FileSystem::ConfigFile configFile(MainModule::GetModuleDirectory(), GU_CONFIG_FILE_NAME);
		configFile.OpenRead();
	...
	bool GenericUsbInput::MatchesDirection(GuJoystick joystick, GuDirection directionEnum, float threshold)
	{
		...
		{
		case ReplNau::Input::GU_DIR_UP:
			return joystick.YAxis <= -threshold;

		case ReplNau::Input::GU_DIR_RIGHT:
			return joystick.XAxis >= +threshold;

		case ReplNau::Input::GU_DIR_DOWN:
			return joystick.YAxis >= +threshold;

		case ReplNau::Input::GU_DIR_LEFT:
			return joystick.XAxis <= -threshold;
